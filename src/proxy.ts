...
export async function proxy(request: NextRequest) {
  ...
  if (isAdminRoute(pathname)) {
    // Single source of truth: read role from the Prisma DB (see
    // requireAdmin in lib/auth.ts). Demo sessions are never admin.
    const isDemo = user.email?.endsWith("@tradcopilot.local") === true;
    if (!isDemo) {
      // Import happen lazily to avoid a cycle with lib/auth → lib/prisma.
      const { requireAdmin } = await import("@/lib/auth");
      const adminCheck = await requireAdmin(user.id, false);
      if (!adminCheck.ok) {
        const url = request.nextUrl.clone();
