 ...
 *
 *  V4.1: persists a full deterministic market snapshot so the Decision
 *  Record can honestly reconstruct what was known at decision time. */
export async function createThesis(input: CreateThesisInput) {
  ...
  // V4.1: build the full creation-time market snapshot deterministically.
  // This is the THEN state that the Decision Record will compare against NOW.
  let snapshot: Awaited<ReturnType<typeof buildMarketSnapshot>>["snapshot"] = null;
  try {
    const result = await buildMarketSnapshot(input.symbol, input.timeframe, {
      bias: setup.direction,
      aiEvidenceFor: input.evidenceFor,
      aiEvidenceAgainst: input.evidenceAgainst,
    });
    if (result.freshness !== "UNAVAILABLE") {
      snapshot = result.snapshot;
    }
  } catch {
    // Snapshot is best-effort: a thesis can still be created if market data
    // is temporarily unavailable, but we will not fabricate a snapshot.
  }

  // V2.5 fallback: if the client didn't carry evidence, use the snapshot's
  // deterministic evidence. If snapshot failed, rebuild from market context.
  let finalEvidenceFor: string[] = input.evidenceFor ?? [];
  let finalEvidenceAgainst: string[] = input.evidenceAgainst ?? [];

  if (finalEvidenceFor.length === 0 || finalEvidenceAgainst.length === 0) {
    if (snapshot) {
      finalEvidenceFor = snapshot.evidence.for.map((e) => e.text);
      finalEvidenceAgainst = snapshot.evidence.against.map((e) => e.text);
    } else {
      try {
        ...
        const fallback = buildEvidence(setup.direction, ctx, {
          rsi: input.telemetry.rsi ?? 50,
        ...
        finalEvidenceFor = fallback.for;
        finalEvidenceAgainst = fallback.against;
      } catch {
        // Evidence is best-effort; an empty trail is still better than fabricating.
      }
    }
  }

  // V4.1: if a snapshot exists, extract the creation price and confidence
  // score for fast display and honest comparison.
  const regimeAtCreation = snapshot?.marketContext.regime ?? null;
  const confidenceScore = snapshot?.confidence.score ?? null;
  const priceAtCreation = snapshot?.priceAtCapture.price ?? input.telemetry.currentPrice ?? null;
  ...
  return prisma.thesis.create({
    data: {
      ...
      confidence: input.analysis.confidence ?? snapshot?.confidence.tier ?? "MEDIUM",
      confidenceScore,
      priceAtCreation,
      ...
      regimeAtCreation,
      evidenceFor: finalEvidenceFor,
      evidenceAgainst: finalEvidenceAgainst,
