...
export function isDemoUser(userId: string, email?: string): boolean {
  ...
  // Demo sessions now use random UUIDs with a @tradcopilot.local email —
  // recognise them by email domain so entitlements resolve correctly.
  if (email && email.endsWith("@tradcopilot.local")) return true;
  return false;
}

/**
 * Resolve a user's plan. Trusts ONLY the database billing state, never the
 * email. Demo users (incl. random-UUID demo sessions) always resolve to
 * YC_DEMO regardless of email.
 */
