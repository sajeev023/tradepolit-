  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <Hairline />
        {/* V4.1 Evidence Panel — structured evidence with contradiction detection. */}
        {analysisData.evidence && (analysisData.evidence.for?.length > 0 || analysisData.evidence.against?.length > 0) && (
          <EvidencePanel
            forItems={analysisData.evidence.for}
            againstItems={analysisData.evidence.against}
        ...
        {(() => {
          ...
          return (
            ...
            >
              {thesisSaving ? (
                <>
                  <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-[#05070B]/30 border-t-[#05070B]" />
                  <span>Committing Decision...</span>
                </>
              ) : thesisSaved ? (
                <>
                  <BookmarkCheck size={14} />
                  <span>Decision Committed · Monitored</span>
                </>
              ) : directional ? (
                <>
                  <BookmarkCheck size={14} />
                  <span>Commit Decision (Save Thesis)</span>
                </>
              ) : (
