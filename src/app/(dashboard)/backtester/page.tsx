    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b pb-3" style={{ borderColor: "var(--color-border-subtle)" }}>
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] uppercase font-mono px-1.5 py-0.5 rounded bg-amber-950/30 text-amber-400 border border-amber-500/20">Demo</span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-[var(--color-bg-tertiary)] p-3 rounded border border-[var(--color-border-subtle)]">
          ...
          <span className="text-lg font-bold font-mono" style={{ color: "var(--color-accent-primary)" }}>{demoMetrics.totalTrades}</span>
        </div>
        <div className="bg-[var(--color-bg-tertiary)] p-3 rounded border border-[var(--color-border-subtle)]">
          <span className="text-[10px] uppercase tracking-wider block" style={{ color: "var(--color-text-tertiary)" }}>Win Rate</span>
          <span className="text-lg font-bold font-mono [var(--color-accent-primary)]">{(demoMetrics.winRate * 100).toFixed(1)}%</span>
        </div>
        <div className="bg-[var(--color-bg-tertiary)] p-3 rounded border border-[var(--color-border-subtle)]">
          <span className="text-[10px] uppercase tracking-wider block" style={{ color: "var(--color-text-tertiary)" }}>Loss Rate</span>
          <span className="text-lg font-bold font-mono [var(--color-accent-primary)]">{(demoMetrics.lossRate * 100).toFixed(1)}%</span>
        </div>
        <div className="bg-[var(--color-bg-tertiary)] p-3 rounded border border-[var(--color-border-subtle)]">
        ...
        <div className="bg-[var(--color-bg-tertiary)] p-3 rounded border border-[var(--color-border-subtle)]">
          ...
          <span className="text-lg font-bold font-mono [var(--color-accent-primary)]">{demoMetrics.profitFactor.toFixed(2)}</span>
        </div>
        <div className="bg-[var(--color-bg-tertiary)] p-3 rounded border border-[var(--color-border-subtle)]">
        ...
        <div className="max-h-[120px] overflow-y-auto space-y-1.5 pr-1">
          {demoTrades.map((t, idx) => (
            <div key={idx} className="bg-[var(--color-bg-tertiary)] p-2 rounded border border-[var(--color-border-subtle)] flex items-center justify-between text-xs font-mono">
              <div>
                <span className={`font-bold mr-2 ${t.direction === 'SHORT' ? 'text-[var(--color-loss)]' : '[var(--color-accent-primary)]'}`}>{t.direction}</span>
...
export default function BacktesterPage() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto">
      {/* Header */}
      <div>
        <p className="tp-eyebrow mb-2">TOOLS · BACKTEST</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card p-5 space-y-4">
            ...
            {isCreatingStrategy ? (
              <form onSubmit={handleCreateStrategy} className="space-y-4 pt-2 border-t" style={{ borderColor: "var(--color-border-subtle)" }}>
                <h3 className="text-xs font-bold [var(--color-accent-primary)]">New Strategy Details</h3>
                <div>
                ...
                {/* Entry parameters */}
                <div className="space-y-2">
                  <span className="block text-[10px] uppercase font-bold [var(--color-accent-primary)]">Entry Buy Signal</span>
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    type="submit"
                    className="flex-1 py-2 rounded text-xs font-semibold bg-cyan-400 text-[var(--color-bg-primary)] hover:bg-cyan-300 transition-colors"
                  >
                    Save Ruleset
                  </button>
                  <button
                ...
                <div>
                  ...
                  {stratLoading ? (
                    <div className="flex justify-center p-4">
                      <RefreshCw className="animate-spin [var(--color-accent-primary)]" size={16} />
                    </div>
                  ...
                  <button
                    ...
                    className="mt-2.5 text-xs font-semibold [var(--color-accent-primary)] hover:text-cyan-300 transition-colors block"
                  >
          ...
          <div className="card p-4 flex gap-3">
            <Info size={16} className="[var(--color-accent-primary)] shrink-0 mt-0.5" />
          ...
          <div className="card p-5 min-h-[400px] flex flex-col justify-between">
            {!activeBacktestId || !activeBacktest ? (
              ...
              <div className="flex flex-col items-center justify-center py-28 text-center my-auto">
                <RefreshCw className="animate-spin [var(--color-accent-primary)] mb-2" size={24} />
                <h3 className="text-sm font-semibold" style={{ color: "var(--color-text-secondary)" }}>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between border-b pb-3" style={{ borderColor: "var(--color-border-subtle)" }}>
                  ...
                  <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-emerald-950/30 text-[var(--color-profit)] border border-emerald-500/20">
                    Complete
                  </span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-[var(--color-bg-tertiary)] p-3 rounded border border-[var(--color-border-subtle)]">
                    ...
                    <span className="text-lg font-bold font-mono [var(--color-accent-primary)]">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="max-h-[120px] overflow-y-auto space-y-1.5 pr-1">
                      {trades.map((t: any, idx: number) => (
                        <div key={idx} className="bg-[var(--color-bg-tertiary)] p-2 rounded border border-[var(--color-border-subtle)] flex items-center justify-between text-xs font-mono">
                          <div>
                            <span className={`font-bold mr-2 ${t.direction === 'SHORT' ? 'text-[var(--color-loss)]' : '[var(--color-accent-primary)]'}`}>
                              {t.direction || 'LONG'}
                            </span>
