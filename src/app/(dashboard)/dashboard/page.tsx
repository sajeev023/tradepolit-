...
function DashboardContent() {
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-2 space-y-4">
                {needReviewTheses.length > 0 && (
                  <div className="rounded-xl border p-4" style={{ borderColor: "rgba(245, 185, 66, 0.30)", backgroundColor: "rgba(245, 185, 66, 0.04)" }}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2 text-xs font-semibold text-[var(--color-warning)] mb-1">
                          <Clipboard size={14} />
                          <span>Outcome Review Required</span>
                          <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-[var(--color-warning)]/20 text-[var(--color-warning)]">
                            {needReviewTheses.length}
                          </span>
                        </div>
                        <p className="text-[11px] text-[var(--color-text-secondary)] max-w-xl">
                          Resolved decisions are waiting for outcome attribution. Closing the loop feeds your calibration and DecisionScore.
                        </p>
                      </div>
                      <Link
                        href="/theses?status=NEEDS_REVIEW"
                        className="shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold text-[#05070B] bg-[var(--color-warning)] hover:brightness-110 transition-all flex items-center gap-1.5"
                      >
                ...
                {openTheses.length > 0 && (
                  <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] p-4 sm:p-5 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Compass size={15} className="text-[var(--color-accent-primary)]" />
                        <h2 className="text-sm font-semibold text-[var(--color-text-primary)]">Active Decisions</h2>
                      ...
                      {openTheses.slice(0, 4).map((t: any) => (
                        ...
                        >
                          ...
                          <div className="pt-1.5 border-t border-[var(--color-border-subtle)] flex items-center justify-between text-[10px] text-[var(--color-text-quaternary)]">
                            ...
                            <span className="text-[var(--color-accent-primary)] group-hover:translate-x-0.5 transition-transform flex items-center gap-0.5">Record <ArrowRight size={10} /></span>
                          </div>
                        </Link>
                      ))}
                    </div>
              ...
              {/* Side decision health */}
              <div className="space-y-4">
                <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] p-4 space-y-3">
                  <h3 className="text-xs font-semibold text-[var(--color-text-primary)] flex items-center gap-1.5">
                    <Activity size={13} className="text-[var(--color-accent-primary)]" /> Decision Health
                  </h3>
                ...
                {resolvedTheses.length > 0 && (
                  <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] p-4 space-y-3">
                    <h3 className="text-xs font-semibold text-[var(--color-text-primary)] flex items-center gap-1.5">
                      <Award size={13} className="text-[var(--color-profit)]" /> Recent Outcomes
                    </h3>
                    <div className="space-y-2">
                      {resolvedTheses.map((t: any) => (
                        <Link key={t.id} href={`/theses/${t.id}`} className="flex items-center justify-between p-2 rounded bg-[var(--color-bg-deepest)]/40 border border-[var(--color-border-subtle)] hover:border-[var(--color-border-strong)] transition-colors group">
      ...
      {/* ── Performance & Learning Row ── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Equity Curve */}
        {summaryLoading ? (
          <ChartSkeleton />
        ) : (
          <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] p-5 lg:col-span-2 flex flex-col justify-between min-h-[340px]">
            <div className="flex items-center justify-between mb-4">
              <div>
                <span className="text-[10px] font-mono uppercase tracking-widest text-[var(--color-text-quaternary)] mb-1 block">Performance</span>
                <h2 className="text-sm font-semibold text-[var(--color-text-primary)]">Equity Curve</h2>
              </div>
              {equityCurveData.length > 0 && (
                <span className="text-[10px] font-mono font-semibold px-2 py-1 rounded bg-[var(--color-accent-primary-subtle)] text-[var(--color-accent-primary)]">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] p-5 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-semibold text-[var(--color-text-primary)] flex items-center gap-1.5">
                <Flame size={13} className="text-[var(--color-warning)]" /> Behavioral Insight
              </h3>
            </div>
          ...
          <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] p-5 space-y-3">
            <h3 className="text-xs font-semibold text-[var(--color-text-primary)] flex items-center gap-1.5">
              <Clipboard size={13} className="text-[var(--color-accent-primary)]" /> Weekly Report
            </h3>
            ...
            <button
              ...
              className="w-full px-3 py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-all"
      ...
      {/* ── Recent Trades & Weekly Report Output ── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/journal" className="text-xs text-[var(--color-accent-primary)] hover:underline font-medium">Journal &rarr;</Link>
          </div>
