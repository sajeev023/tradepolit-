...
export async function POST(request: NextRequest) {
  try {
    ...
    // Expensive endpoint (2000-token AI completion). Cap to prevent quota abuse.
    const rl = checkUserRateLimit(user.id, request, "weekly-report", 5, 60 * 60 * 1000);
    if (!rl.result.allowed) {
      const retryAfterSec = Math.ceil((rl.result.resetAt - Date.now()) / 1000);
      return rateLimitedError(retryAfterSec * 1000, "Weekly report limit reached. Please try again later.");
    }
