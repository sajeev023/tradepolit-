...
export async function POST(request: NextRequest) {
  try {
    ...
    // Per-user rate limit on trade creation (M-6).
    const rl = checkUserRateLimit(user.id, request, "trades-create", 30, 60_000);
    if (!rl.result.allowed) {
      const retryAfterSec = Math.ceil((rl.result.resetAt - Date.now()) / 1000);
      return rateLimitedError(retryAfterSec * 1000, "Too many trade creations. Please slow down.");
    }
