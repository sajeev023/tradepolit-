...
export async function POST(request: NextRequest) {
  ...
  try {
    // Signature verification is mandatory whenever a webhook secret is set.
    // There is no "mock"-prefix shortcut: in production, a missing/invalid
    // secret must fail rather than silently accept unverified payloads
    // (which would let an attacker forge checkout.session.completed and
    // upgrade arbitrary users to PRO).
    if (webhookSecret) {
