...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({ ...dict.cryptoMarketAnalysis.meta, path: "/crypto-market-analysis", locale });
}

export default async function LocaleCryptoMarketAnalysisPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: localeParam } = await params;
  const locale = parseLocale(localeParam) as SupportedLocale;
  if (!locale || locale === "en") notFound();

  const dict = await getDictionary(locale);
  const d = dict.cryptoMarketAnalysis;
  const lp = (path: string) => localePath(locale, path);
  const BREADCRUMBS = [{ name: d.meta.title.split("—")[0].trim(), path: "/crypto-market-analysis" }];
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--ink)] font-sans antialiased">
      ...
      <main className="tc-section">
        <div className="max-w-[900px] mx-auto px-4 sm:px-6 space-y-12 sm:space-y-16">
          ...
          <section className="space-y-4"><h2 className="tp-h2">{d.liveDataHeading}</h2></section>
          <hr className="tc-rule" />
          <section className="space-y-4"><h2 className="tp-h2">{d.analysisHeading}</h2></section>
          <hr className="tc-rule" />
          <section className="space-y-4"><h2 className="tp-h2">{d.sentimentHeading}</h2></section>
          <hr className="tc-rule" />
