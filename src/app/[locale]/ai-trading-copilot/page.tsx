...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({ ...dict.aiTradingCopilot.meta, path: "/ai-trading-copilot", locale });
}

export default async function LocaleAiTradingCopilotPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: localeParam } = await params;
  const locale = parseLocale(localeParam) as SupportedLocale;
  if (!locale || locale === "en") notFound();

  const dict = await getDictionary(locale);
  const d = dict.aiTradingCopilot;
  const lp = (path: string) => localePath(locale, path);
  const BREADCRUMBS = [{ name: d.h1 + " " + d.h1Italic, path: "/ai-trading-copilot" }];
      ...
      <main className="tc-section">
        <div className="max-w-[900px] mx-auto px-4 sm:px-6 space-y-12 sm:space-y-16">
          ...
          <section className="space-y-4">
            <h2 className="tp-h2">{d.definitionHeading}</h2>
            <p className="tp-body text-[14px] leading-relaxed">{d.definitionBody}</p>
          </section>
          ...
          {/* Limits */}
          <section className="space-y-4">
            <h2 className="tp-h2">{d.limitsHeading}</h2>
          </section>
